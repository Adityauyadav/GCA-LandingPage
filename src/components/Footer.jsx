import logo from '../assets/logo.svg';

const Footer = () => {
  const scrollToTop = () => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const navLinks = [
    'Home', 'About', 'Membership', 'Contact', 
    'Terms and Conditions', 'Privacy Policy', 
    'Cancellation and Refund', 'Shipping and Exchange'
  ];

  return (
    <footer className="bg-black text-white py-16 px-8 font-sans">
      <div className="max-w-7xl mx-auto">
        {/* Header Text */}
        <div className="text-center mb-16">
          <p className="text-2xl font-light">
            Powered by <span className="font-bold">IdeaTribe</span>
          </p>
        </div>

        {/* Main Grid */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16 text-sm">
          {/* Logo Section */}
          <div className="flex justify-start items-start">
            <img src={logo} alt="GCA Logo" className="h-10 w-auto" />
          </div>

          {/* Quick Navigation */}
          <div>
            <h3 className="text-base font-bold mb-6 uppercase tracking-wider">Quick Navigation</h3>
            <ul className="space-y-3 text-gray-400">
              {navLinks.map((link) => (
                <li key={link}>
                  <a href={`#${link.toLowerCase().replace(/\s+/g, '-')}`} className="hover:text-white transition-colors">
                    {link}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Stay Connected */}
          <div>
            <h3 className="text-base font-bold mb-6 uppercase tracking-wider">Stay Connected</h3>
            <div className="flex flex-col gap-4">
               {/* YouTube Link */}
               <a href="#" className="flex items-center gap-3 text-white  transition-colors group" aria-label="YouTube">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" >
                      <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                  </svg>
                  <span className="text-gray-400 group-hover:text-white">Youtube</span>
               </a>
               
               {/* LinkedIn Link */}
               <a href="#" className="flex items-center gap-3 text-gray-400 hover:text-[#0077b5] transition-colors group" aria-label="LinkedIn">
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="group-hover:fill-[#0077b5] transition-colors">
                      <path d="M16.6699 0C17.4045 0.000122745 17.9998 0.577586 18 1.28906V16.7109C17.9999 17.4227 17.4045 17.9999 16.6699 18H1.33008C0.595772 17.9999 0 17.4226 0 16.7109V1.28906C0.000183056 0.577586 0.595814 0.000122862 1.33008 0H16.6699ZM12.082 6.77051C10.6527 6.77051 10.0122 7.5563 9.6543 8.1084V6.95996H6.96094C6.99592 7.71643 6.96128 14.9918 6.96094 15.0635H9.6543V10.5391C9.6543 10.2972 9.67204 10.0546 9.74316 9.88184C9.93779 9.39774 10.3808 8.89649 11.125 8.89648C12.0993 8.89648 12.4892 9.63927 12.4893 10.7285V15.0635H15.1826V10.417C15.1825 7.92827 13.854 6.77053 12.082 6.77051ZM2.77637 15.0635H5.46973V6.95996H2.77637V15.0635ZM4.1416 3.05371C3.21976 3.05373 2.61816 3.65881 2.61816 4.4541C2.61829 5.23164 3.20253 5.85352 4.10645 5.85352H4.12402C5.06291 5.85341 5.64737 5.23161 5.64746 4.4541C5.62988 3.65873 5.06284 3.05371 4.1416 3.05371Z" fill="white"/>
                  </svg>
                  <span className="text-gray-400 group-hover:text-white">Linkedin</span>
               </a>
            </div>
          </div>

          {/* Get In Touch */}
          <div>
            <h3 className="text-base font-bold mb-6 uppercase tracking-wider">Get In Touch</h3>
            <ul className="space-y-4 text-gray-400">
              <li>
                <a href="mailto:team@gcaforum.com" className="hover:text-white underline decoration-zinc-700 underline-offset-4">
                  team@gcaforum.com
                </a>
              </li>
              <li>
                <span className="block">Phone No. +91 8910392803</span>
              </li>
            </ul>
          </div>
        </div>

        {/* Scroll to Top Button */}
        <div className="flex justify-center mb-10">
          <button 
            onClick={scrollToTop}
            className="bg-blue-600 hover:bg-blue-700 p-4 rounded-full transition-all duration-300"
            aria-label="Scroll to top"
          >
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
              <path d="m18 15-6-6-6 6"/>
            </svg>
          </button>
        </div>

        {/* Bottom Copyright */}
        <div className="border-t border-zinc-900 pt-8 text-center text-xs text-zinc-500 uppercase tracking-widest">
          © 2025 Global Cybersecurity Association. All Rights Reserved.
        </div>
      </div>
    </footer>
  );
};

export default Footer;